...
if (process.argv.length < 6) {
    console.error("node Liqi tagert proxy rate time");
    process.exit(1);
}
...
async function openBrowser(targetURL, browserProxy) {
    ...
    try {
        ...
        console.log("LIQI BROWSER Fail Solved: " + browserProxy);
        await browser.close();
    }
}
        ...
        try {
            ...
            if (response) {
                if (response.title === "Just a moment...") {
                    console.log("LIQI BROWSER solve : " + browserProxy + " - Bypass failed ! ");
