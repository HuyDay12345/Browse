...
async function detectChallenge(browser, page, browserProxy) {
    ...
    if (content.includes("challenge-platform")) {
        console.log("HTTP-MOGINA Start Bypass: " + browserProxy);

        try {
            await sleep(15);
            // Ch? cho ph?n t? Captcha xu?t hi?n và sau dó th?c hi?n click
    ...
    console.log("HTTP-MOGINA Can't Find ! " + browserProxy);
    await sleep(10);
}
...
async function openBrowser(targetURL, browserProxy) {
    ...
    try {
        ...
        console.error('Error run HTTP-MOGINA');
        return;
    }
  
    try {
        console.log("HTTP-MOGINA Start Solved : " + browserProxy);
        ...
        console.error('Error while run HTTP-MOGINA');
    } finally {
        console.log("HTTP-MOGINA Fail Solved: " + browserProxy);
        await browser.close();
    }
}
